...
func TestParagraphAndSection(t *testing.T) {
	...
	if strings.Index(p, "PROJECT SESSIONS") > strings.Index(p, "AGENTS ON THIS BUS") {
		t.Error("sessions come first: that is where the project is")
	}
	s := Section(sessions, agents)
	for _, want := range []string{"Project sessions in this window", "- hq: ", "Agents (message by name", "kb-writer"} {
		if !strings.Contains(s, want) {
			t.Errorf("section missing %q:\n%s", want, s)
		}
	}
	// Each half stands alone: a window with repos and no agents, or the reverse.
	if p := Paragraph(sessions, nil); !strings.Contains(p, "backend") || strings.Contains(p, "AGENTS ON") {
