...
func TestStartupProgram(t *testing.T) {
	cases := []struct{ cmd, want string }{
		...
		{"/home/u/.local/bin/ccmuxd env-exec -f /w/shared/.env -f /w/a/.env -- CLAUDE_PEERS_NAME=x opencode --agent x", "opencode"},
		{"ccmuxd env-exec -f /w/.env -- CLAUDE_PEERS_NAME=x env -u TMUX claude --name x", "claude"},
		{"ccmuxd install --hub", "ccmuxd"},
		{"npm run dev", "npm"},
