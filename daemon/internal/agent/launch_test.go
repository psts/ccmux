...
func TestLaunchCommandOpencode(t *testing.T) {
	...
	l := LaunchCommand(d, harness.Harness{Name: "opencode", Command: "opencode"}, "/b", []string{"/repo"}, "post about it's release", 41234)
...
func TestLaunchCommandClaude(t *testing.T) {
	...
	l := LaunchCommand(d, h, "/home/u/.ccmux/agents/kb-writer", []string{"/repo", "/repo2"}, "write it", 0)
	...
	def := LaunchCommand(withDefaults(d), h, "/b", nil, "", 0).Persist
	...
	got := LaunchCommand(locked, h, "/b", nil, "", 0).Persist
...
func TestLaunchCommandOtherHarness(t *testing.T) {
	l := LaunchCommand(Definition{Name: "a-b"}, harness.Harness{Name: "pi", Command: "pi"}, "/b", nil, "hi there", 0)
