...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    public override func keyDown(with event: NSEvent) {
        ...
        if !terminal.keyboardEnhancementFlags.isEmpty {
            ...
            // Skip kitty encoder for Cmd+key — let interpretKeyEvents/doCommand
            // handle macOS selectors (e.g. Cmd+Arrow → word movement via ESC b/f)
    ...
    public override func doCommand(by selector: Selector) {
        if !terminal.keyboardEnhancementFlags.isEmpty {
            // Extract modifiers from the original NSEvent stored before interpretKeyEvents,
            // same pattern as insertText uses via pendingKittyKeyEvent.
            let pending = pendingKittyKeyEvent
            pendingKittyKeyEvent = nil
            let mods: KittyKeyboardModifiers
            let evtType: KittyKeyboardEventType
            if let pending {
                mods = kittyModifiers(from: pending.event, includeOption: optionAsMetaKey)
                evtType = pending.eventType
            } else {
                mods = []
                evtType = .press
            }
            ...
            case #selector(insertNewline(_:)):
                if sendKittyFunctionalKey(.enter, modifiers: mods, eventType: evtType) { return }
            case #selector(cancelOperation(_:)):
                if sendKittyFunctionalKey(.escape, modifiers: mods, eventType: evtType) { return }
            case #selector(deleteBackward(_:)):
                if sendKittyFunctionalKey(.backspace, modifiers: mods, eventType: evtType) { return }
            case #selector(moveUp(_:)):
                if sendKittyFunctionalKey(.up, modifiers: mods, eventType: evtType) { return }
            case #selector(moveDown(_:)):
                if sendKittyFunctionalKey(.down, modifiers: mods, eventType: evtType) { return }
            case #selector(moveLeft(_:)):
                if sendKittyFunctionalKey(.left, modifiers: mods, eventType: evtType) { return }
            case #selector(moveRight(_:)):
                if sendKittyFunctionalKey(.right, modifiers: mods, eventType: evtType) { return }
            case #selector(insertTab(_:)):
                if sendKittyFunctionalKey(.tab, modifiers: mods, eventType: evtType) { return }
            case #selector(insertBacktab(_:)):
                if sendKittyFunctionalKey(.tab, modifiers: mods.union(.shift), eventType: evtType) { return }
            case #selector(moveToBeginningOfLine(_:)):
                if sendKittyFunctionalKey(.home, modifiers: mods, eventType: evtType) { return }
            case #selector(moveToEndOfLine(_:)):
                if sendKittyFunctionalKey(.end, modifiers: mods, eventType: evtType) { return }
            case #selector(scrollPageUp(_:)):
                fallthrough
            case #selector(pageUp(_:)):
                if terminal.applicationCursor {
                    if sendKittyFunctionalKey(.pageUp, modifiers: mods, eventType: evtType) { return }
                } else {
                    pageUp()
                    return
                }
            ...
            case #selector(pageDown(_:)):
                if terminal.applicationCursor {
                    if sendKittyFunctionalKey(.pageDown, modifiers: mods, eventType: evtType) { return }
                } else {
                    pageDown()
                    return
                }
    ...
    private func kittyFunctionalKey(from event: NSEvent) -> KittyFunctionalKey? {
        ...
        // Note: macOS always sets .numericPad on regular arrow/navigation keys.
        // Actual numpad keys (0-9, +, -, etc.) are handled by keyCode above.
        // There are no separate keypad keyCodes for arrows on macOS, so skip
        // the .numericPad check and always use the regular key variants.
    ...
    /// Points of precise (trackpad) scrolling to accumulate before emitting one
    /// page when forwarding the wheel to an alternate-buffer app. Tuned by feel —
    /// raise it if trackpad scrolling pages too fast, lower it if too slow.
    static let wheelPageThreshold: CGFloat = 50

    /// Running sum of precise scroll deltas while forwarding the wheel to an
    /// alternate-buffer app (e.g. Claude Code's fullscreen TUI) as PgUp/PgDn.
    private var wheelPageAccumulator: CGFloat = 0

    /// Pure page-stepping math for trackpad wheel forwarding (factored out for
    /// testing). Folds `delta` into `accumulator`, resetting first if the gesture
    /// reversed so a flick back responds at once, then peels off whole pages.
    /// - Returns: signed page count (positive = up / PgUp) and the new accumulator.
    public static func wheelPageSteps (accumulator: CGFloat, delta: CGFloat, threshold: CGFloat) -> (pages: Int, accumulator: CGFloat) {
        if delta == 0 {
            return (0, accumulator)
        }
        var acc = accumulator
        // Reset when the gesture reverses direction.
        if (delta > 0) != (acc > 0) {
            acc = 0
        }
        acc += delta
    ...
    public override func scrollWheel(with event: NSEvent) {
        ...
        // On the alternate buffer there is no local scrollback to move. Rather
        // than let the wheel do nothing, forward it to the app as PgUp/PgDn —
        // the same sequence the PgUp/PgDn keys send there — so full-screen TUIs
        // like Claude Code scroll their own view under the wheel.
        if terminal.isDisplayBufferAlternate {
            forwardWheelToAlternate (with: event)
            return
        }
    ...
    private func forwardWheelToAlternate (with event: NSEvent) {
        // Discrete mouse wheel (no precise deltas): one page per notch.
        guard event.hasPreciseScrollingDeltas else {
            send (event.deltaY > 0 ? EscapeSequences.cmdPageUp : EscapeSequences.cmdPageDown)
            return
        }
        // Trackpad: ignore inertial momentum so a flick doesn't rocket through
        // pages, and accumulate active-finger deltas into whole-page steps.
        if event.momentumPhase != [] {
            return
        }
        let (pages, acc) = TerminalView.wheelPageSteps (
            accumulator: wheelPageAccumulator,
            delta: event.scrollingDeltaY,
            threshold: TerminalView.wheelPageThreshold)
        wheelPageAccumulator = acc
        let seq = pages > 0 ? EscapeSequences.cmdPageUp : EscapeSequences.cmdPageDown
        for _ in 0 ..< abs (pages) {
            send (seq)
        }
    }
