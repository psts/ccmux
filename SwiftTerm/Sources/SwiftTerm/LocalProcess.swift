...
public class LocalProcess {
    ...
    public func terminate()
    {
        ...
        if shellPid != 0 {
            // Kill the shell's whole process group, not just the shell itself.
            // forkpty calls setsid, so shellPid is the pgroup leader and pgid == shellPid;
            // getpgid is the defensive path in case setsid raced or the shell already died.
            let pgid = getpgid(shellPid)
            let pg = pgid > 0 ? pgid : shellPid
            killpg(pg, SIGTERM)
